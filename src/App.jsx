...
export const utils = {
  ...
  openWhatsAppBusiness: (phone, msg) => { const text = encodeURIComponent(msg); window.open(`https://wa.me/${phone}?text=${text}`, '_blank'); }
};
const { getSafeLocal, setSafeLocal, triggerHaptic, safeNum, formatTime12h, openWhatsAppBusiness } = utils;
...
const EventCardItem = React.memo(({ ev, idx, todayObj, onWhatsApp, onViewDoc, onEdit, onDelete, onMapClick, empresa }) => {
    ...
    const handleTouchEnd = useCallback(() => { setIsDragging(false); if (swipeX > 80) { triggerHaptic('success'); onDelete(ev.id); } setSwipeX(0); }, [swipeX, ev.id, onDelete]);
...
export default function App() {
  ...
  const renderFinanzas = () => {
      const cy = todayObj.getFullYear();
      const cm = todayObj.getMonth() + 1;
  ...
  if (!isAuthenticated) return (
    <div className="font-outfit min-h-screen flex items-center justify-center p-4 bg-slate-950">
      <div className="w-full max-w-sm bg-slate-900/60 backdrop-blur-2xl rounded-[2.5rem] p-10 shadow-[0_20px_50px_rgba(0,0,0,0.5)] border border-white/10 relative z-10 animate-fadeInUp">
        <div className="flex justify-center mb-8">
          <div className="w-20 h-20 rounded-[1.5rem] bg-gradient-to-tr from-violet-600 to-indigo-500 p-[2px] shadow-lg hover:scale-105 transition-transform duration-500">
            <div className="w-full h-full bg-slate-900 rounded-[1.4rem] flex items-center justify-center">
      ...
      {isDescModalOpen && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[10000] flex justify-center items-center p-4 animate-fadeIn">
          <div className="bg-white/90 dark:bg-slate-900/90 backdrop-blur-2xl p-6 sm:p-8 rounded-[2rem] shadow-2xl border border-white/50 dark:border-slate-700/50 max-w-md w-full transition-all duration-300 ease-out">
            ...
            <textarea value={descEditData.desc} onChange={e => setDescEditData({...descEditData, desc: e.target.value})} className="w-full bg-slate-50/50 dark:bg-slate-800/50 backdrop-blur-md border border-slate-200/50 dark:border-slate-700/50 rounded-2xl p-4 text-sm font-medium outline-none focus:border-violet-500 transition-all duration-300 ease-out mb-6 h-32 resize-none shadow-inner" placeholder="Escribe los detalles aquí..." />
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-[9998] bg-slate-900/60 backdrop-blur-sm flex justify-center items-end sm:items-center p-0 sm:p-4 animate-fadeIn">
          <div className="bg-white/90 dark:bg-slate-900/90 backdrop-blur-3xl w-full h-[92vh] sm:h-auto sm:max-h-[90vh] sm:max-w-2xl rounded-t-[2.5rem] sm:rounded-[2.5rem] flex flex-col overflow-hidden shadow-[0_20px_50px_rgba(0,0,0,0.3)] animate-slideUp relative transition-all duration-300 ease-out border border-white/50 dark:border-slate-700/50">
             ...
             <div className="md:hidden w-full flex justify-center pt-4 pb-2 bg-transparent rounded-t-[2.5rem]">
             ...
             <div className="p-6 border-b border-slate-200/50 dark:border-slate-800/50 flex justify-between items-center bg-transparent z-20">
                <h3 className="font-black text-slate-800 dark:text-white text-2xl flex items-center gap-2 tracking-tight">
                    {currentEvento ? <Edit size={28} className="text-violet-500 drop-shadow-md"/> : (isCotizacionMode ? <FileText size={28} className="text-amber-500 drop-shadow-md"/> : <Plus size={28} className="text-violet-500 drop-shadow-md"/>)} 
                    {currentEvento ? 'Editar Reserva' : (isCotizacionMode ? 'Cotización Rápida' : 'Nueva Reserva')}
                </h3>
                <div className="flex gap-2">
                   {!currentEvento && !isCotizacionMode && (<button type="button" onClick={clearDraft} className="p-3 bg-rose-50/50 hover:bg-rose-100 text-rose-600 dark:bg-rose-500/10 dark:hover:bg-rose-500/20 rounded-2xl transition-all duration-300 ease-out hover:scale-105 active:scale-95 border border-transparent hover:border-rose-200/50" title="Limpiar formulario"><Trash2 size={20}/></button>)}
              ...
              <form onSubmit={handleSaveEvento} className="max-w-xl mx-auto pb-8" noValidate>
                 ...
                 <div className="bg-white/50 dark:bg-slate-800/30 backdrop-blur-md rounded-3xl shadow-sm border border-slate-200/50 dark:border-slate-700/50 p-6 mb-6 transition-all duration-300 ease-out hover:shadow-md hover:-translate-y-0.5">
                    ...
                    <div className="space-y-5">
                       <div>
                           <label className="block text-[11px] uppercase text-slate-400 font-black tracking-widest mb-2">Nombre Completo *</label>
                           <input required value={formData.cliente||''} onChange={e=>setFormData({...formData,cliente:e.target.value})} className="w-full bg-white/60 dark:bg-slate-800/60 backdrop-blur-md border border-white/40 dark:border-slate-700/50 rounded-2xl p-4 text-base font-bold outline-none focus:border-violet-500 focus:bg-white dark:focus:bg-slate-800 transition-all duration-300 ease-out capitalize shadow-sm" />
                       </div>
                       <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                          <div>
                              <label className="block text-[11px] uppercase text-slate-400 font-black tracking-widest mb-2">Teléfono *</label>
                              <input required type="tel" value={formData.telefono||''} onChange={e=>setFormData({...formData,telefono:e.target.value})} className="w-full bg-white/60 dark:bg-slate-800/60 backdrop-blur-md border border-white/40 dark:border-slate-700/50 rounded-2xl p-4 text-base font-bold outline-none focus:border-violet-500 focus:bg-white dark:focus:bg-slate-800 transition-all duration-300 ease-out shadow-sm" />
                          </div>
                          <div>
                              <label className="block text-[11px] uppercase text-slate-400 font-black tracking-widest mb-2">Correo (Opcional)</label>
                              <input type="email" value={formData.email||''} onChange={e=>setFormData({...formData,email:e.target.value})} className="w-full bg-white/60 dark:bg-slate-800/60 backdrop-blur-md border border-white/40 dark:border-slate-700/50 rounded-2xl p-4 text-base font-bold outline-none focus:border-violet-500 focus:bg-white dark:focus:bg-slate-800 transition-all duration-300 ease-out shadow-sm" />
                          </div>
                       </div>
                       <div>
                           <label className="block text-[11px] uppercase text-slate-400 font-black tracking-widest mb-2">RUC de Empresa (Opcional)</label>
                           <input type="text" value={formData.ruc||''} onChange={e=>setFormData({...formData,ruc:e.target.value})} className="w-full bg-white/60 dark:bg-slate-800/60 backdrop-blur-md border border-white/40 dark:border-slate-700/50 rounded-2xl p-4 text-base font-bold outline-none focus:border-violet-500 focus:bg-white dark:focus:bg-slate-800 transition-all duration-300 ease-out uppercase shadow-sm" />
                       </div>
                    </div>
                 </div>
                 ...
                 <div className="bg-white/50 dark:bg-slate-800/30 backdrop-blur-md rounded-3xl shadow-sm border border-slate-200/50 dark:border-slate-700/50 p-6 mb-6 transition-all duration-300 ease-out hover:shadow-md hover:-translate-y-0.5">
                    ...
                    <div className="grid grid-cols-2 gap-5 mb-5">
                       <div>
                           <label className="block text-[11px] uppercase text-slate-400 font-black tracking-widest mb-2">Fecha *</label>
                           <input required type="date" value={formData.fecha||''} onChange={e=>setFormData({...formData,fecha:e.target.value})} className="w-full bg-white/60 dark:bg-slate-800/60 backdrop-blur-md border border-white/40 dark:border-slate-700/50 rounded-2xl p-4 text-base font-bold outline-none focus:border-violet-500 focus:bg-white dark:focus:bg-slate-800 transition-all duration-300 ease-out cursor-pointer shadow-sm" />
                       </div>
                       <div>
                           <label className="block text-[11px] uppercase text-slate-400 font-black tracking-widest mb-2">Hora *</label>
                           <input required type="time" value={formData.hora||''} onChange={e=>setFormData({...formData,hora:e.target.value})} className="w-full bg-white/60 dark:bg-slate-800/60 backdrop-blur-md border border-white/40 dark:border-slate-700/50 rounded-2xl p-4 text-base font-bold outline-none focus:border-violet-500 focus:bg-white dark:focus:bg-slate-800 transition-all duration-300 ease-out cursor-pointer shadow-sm" />
                       </div>
                    </div>
                    <div className="mb-5">
                        <label className="block text-[11px] uppercase text-slate-400 font-black tracking-widest mb-2">Zona de Transporte</label>
                        <select value={formData.ubicacion||'Panamá Centro'} onChange={handleZoneChange} className="w-full bg-white/60 dark:bg-slate-800/60 backdrop-blur-md border border-white/40 dark:border-slate-700/50 rounded-2xl p-4 text-base font-bold outline-none focus:border-violet-500 focus:bg-white dark:focus:bg-slate-800 transition-all duration-300 ease-out cursor-pointer shadow-sm">
                            {Object.keys(ZONAS_TRANSPORTE).map(z => <option key={z} value={z}>{z}</option>)}
                        </select>
                    </div>
                    <div>
                        <label className="block text-[11px] uppercase text-slate-400 font-black tracking-widest mb-2">Dirección Exacta</label>
                        <input value={formData.direccion||''} onChange={e=>setFormData({...formData,direccion:e.target.value})} className="w-full bg-white/60 dark:bg-slate-800/60 backdrop-blur-md border border-white/40 dark:border-slate-700/50 rounded-2xl p-4 text-base font-bold outline-none focus:border-violet-500 focus:bg-white dark:focus:bg-slate-800 transition-all duration-300 ease-out shadow-sm" />
                    </div>
                 </div>
                 ...
                 <div className="bg-white/50 dark:bg-slate-800/30 backdrop-blur-md rounded-3xl shadow-sm border border-slate-200/50 dark:border-slate-700/50 p-6 mb-6 transition-all duration-300 ease-out hover:shadow-md hover:-translate-y-0.5">
                    ...
                    <div className="flex flex-wrap gap-2 mb-4">
                       {PAQUETES_DIVERTY.map(p => { 
                          ...
                          return (
                            <button type="button" key={p.id} onClick={()=>addService(p)} className={`px-4 py-2.5 rounded-xl font-black text-[10px] uppercase tracking-widest transition-all duration-300 ease-out border ${isSel ? 'bg-gradient-to-tr from-violet-600 to-indigo-600 text-white shadow-md border-transparent scale-105' : 'bg-white/80 dark:bg-slate-800/80 backdrop-blur-md text-slate-600 dark:text-slate-300 border-slate-200 dark:border-slate-700 hover:border-violet-300 hover:-translate-y-0.5 shadow-sm'}`}>
                                {p.short}
                            </button> 
                          )
                       })}
                    ...
                    <div>
                        <label className="block text-[11px] uppercase text-slate-400 font-black tracking-widest mb-2">Resumen de Paquetes</label>
                        <input type="text" readOnly value={formData.servicio||''} className="w-full bg-slate-50 dark:bg-slate-800/80 border border-slate-200/50 dark:border-slate-700/50 rounded-2xl p-4 text-sm font-bold text-slate-500 outline-none shadow-inner" />
                    </div>
                    <div className="mt-5">
                        <label className="block text-[11px] uppercase text-slate-400 font-black tracking-widest mb-2">Notas Internas (No visibles en PDF)</label>
                        <textarea value={formData.comentarios||''} onChange={e=>setFormData({...formData,comentarios:e.target.value})} className="w-full bg-white/60 dark:bg-slate-800/60 backdrop-blur-md border border-white/40 dark:border-slate-700/50 rounded-2xl p-4 text-base font-bold outline-none focus:border-violet-500 focus:bg-white dark:focus:bg-slate-800 transition-all duration-300 ease-out resize-none h-20 shadow-sm" placeholder="Ej: Llevar extensión extra..." />
                    </div>
                 </div>
                 ...
                 <div className="bg-white/50 dark:bg-slate-800/30 backdrop-blur-md rounded-3xl shadow-sm border border-slate-200/50 dark:border-slate-700/50 p-6 mb-8 transition-all duration-300 ease-out hover:shadow-md hover:-translate-y-0.5">
                    ...
                    <div className="grid grid-cols-2 gap-5 mb-6">
                       <div>
                           <label className="block text-[11px] uppercase text-slate-400 font-black tracking-widest mb-2">Abono ($)</label>
                           <input type="number" inputMode="decimal" value={formData.abono||''} onChange={e=>setFormData({...formData,abono:e.target.value})} className="w-full bg-white/60 dark:bg-slate-800/60 backdrop-blur-md border border-white/40 dark:border-slate-700/50 rounded-2xl p-4 text-lg font-black outline-none focus:border-emerald-500 focus:bg-white dark:focus:bg-slate-800 text-emerald-600 transition-all duration-300 ease-out shadow-sm" />
                       </div>
                       <div>
                           <label className="block text-[11px] uppercase text-slate-400 font-black tracking-widest mb-2">Viáticos ($)</label>
                           <input type="number" inputMode="decimal" value={formData.transporte||''} onChange={e=>setFormData({...formData,transporte:e.target.value, total:(utils.safeNum(formData.total)+(utils.safeNum(e.target.value)-utils.safeNum(formData.transporte))).toString()})} className="w-full bg-white/60 dark:bg-slate-800/60 backdrop-blur-md border border-white/40 dark:border-slate-700/50 rounded-2xl p-4 text-lg font-black outline-none focus:border-emerald-500 focus:bg-white dark:focus:bg-slate-800 transition-all duration-300 ease-out shadow-sm" />
                       </div>
                    </div>
                    <div className="mb-6">
