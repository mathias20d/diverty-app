...
const IconBox = React.memo(({ icon: Icon, color = 'blue', className = '' }) => {
    const c = { blue: 'bg-blue-500/10 text-blue-400 border-blue-500/20', rose: 'bg-rose-500/10 text-rose-400 border-rose-500/20', amber: 'bg-amber-500/10 text-amber-400 border-amber-500/20', emerald: 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20', teal: 'bg-teal-500/10 text-teal-400 border-teal-500/20' };
    return <div className={`p-2.5 rounded-xl border ${c[color]||c.blue} ${className}`}><Icon size={20}/></div>;
});

const Badge = React.memo(({ children, color = 'blue', className = '' }) => {
    const c = { blue: 'bg-blue-500/10 text-blue-400 border-blue-500/20', rose: 'bg-rose-500/10 text-rose-400 border-rose-500/20', amber: 'bg-amber-500/10 text-amber-400 border-amber-500/20', amberSolid: 'bg-amber-300/10 text-amber-300 border-amber-300/20', emerald: 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20', teal: 'bg-teal-500/10 text-teal-400 border-teal-500/20', gray: 'bg-gray-500/10 text-gray-400 border-gray-500/20' };
    return <span className={`border px-3 py-1 rounded-[10px] text-[10px] sm:text-[11px] font-bold uppercase tracking-wider flex items-center gap-1.5 shrink-0 shadow-sm ${c[color]||c.blue} ${className}`}>{children}</span>;
});

const Field = React.memo(React.forwardRef(({ label, as = 'input', className = '', ...props }, ref) => (
    <div className={className}>
        {label && <label className={labelClass}>{label}</label>}
        {as === 'input' && <input ref={ref} className={inputClass} {...props} />}
        {as === 'textarea' && <textarea ref={ref} className={`${inputClass} min-h-[80px] resize-none leading-relaxed`} {...props} />}
        {as === 'select' && <select ref={ref} className={`${inputClass} appearance-none cursor-pointer`} {...props}>{props.children}</select>}
    </div>
)));

const ActionBtn = React.memo(({ icon: Icon, label, color = 'white', onClick }) => {
    const c = { white: 'text-white/70 hover:text-white/90 bg-white/5 hover:bg-white/10', blue: 'text-blue-400/90 hover:text-blue-400 bg-blue-500/10 hover:bg-blue-500/20', rose: 'text-rose-400/90 hover:text-rose-400 bg-rose-500/10 hover:bg-rose-500/20' };
    return <button onClick={onClick} className={`flex-1 font-semibold py-3 flex items-center justify-center gap-2 text-xs uppercase tracking-wider active:scale-[0.98] transition-colors duration-200 rounded-xl ${c[color]}`}><Icon size={16} strokeWidth={2}/> {label}</button>;
});

const PdfItem = ({ label, value, className = '' }) => (
...
const AnimatedProgress = React.memo(({ value }) => {
  ...
  return (
    <div ref={barRef} className="h-full rounded-full transition-all duration-1000 ease-out relative overflow-hidden bg-[#060B14] shadow-inner" style={{ width: `${width}%` }}>
      <div className="absolute inset-0 bg-gradient-to-r from-blue-500 to-indigo-500"></div><div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/20 to-transparent w-[200%] animate-[shimmer_2s_infinite]"></div>
    </div>
  );
});

const SkeletonCard = React.memo(() => ( 
    <div className={`${GLASS_CARD} p-6 animate-pulse flex flex-col gap-4 h-[280px]`}><div className="flex justify-between w-full"><div className="h-5 bg-white/5 rounded-full w-1/3"></div><div className="h-6 bg-white/5 rounded-xl w-16"></div></div><div className="h-10 bg-white/5 rounded-full w-3/4 mt-3"></div><div className="space-y-4 mt-4"><div className="h-4 bg-white/5 rounded-full w-1/2"></div><div className="h-4 bg-white/5 rounded-full w-2/3"></div></div><div className="mt-auto h-14 bg-[#0F172A] rounded-[16px] w-full"></div></div> 
));
...
const EmptyState = React.memo(({ icon: Icon, title, message, actionBtn }) => (
    <div className={`${GLASS_CARD} p-10 text-center flex flex-col items-center justify-center animate-fadeIn w-full border-dashed border-2 border-white/10 min-h-[300px]`}>
        <div className={`w-24 h-24 rounded-[24px] flex items-center justify-center mb-6 border border-white/5 relative overflow-hidden bg-[#0F172A] rotate-3 transition-transform hover:rotate-0 duration-300`}><Icon size={48} strokeWidth={1.5} className="relative z-10 text-white/40 drop-shadow-md" /></div>
...
const AppButton = React.memo(({ children, variant = 'primary', icon: Icon, onClick, className = '', ...props }) => {
    ...
    const vS = { warning: "bg-amber-500/10 text-amber-400 hover:bg-amber-500/20 border border-amber-500/20", success: "bg-emerald-500/10 text-emerald-400 hover:bg-emerald-500/20 border border-emerald-500/20", danger: "bg-rose-500/10 text-rose-400 hover:bg-rose-500/20 border border-rose-500/20", primary: "bg-gradient-to-r from-blue-600 to-indigo-600 text-white shadow-lg shadow-blue-900/20", default: "bg-white/5 text-white/90 hover:bg-white/10" };
    return (<button type="button" onClick={onClick} className={`${baseStyle} ${vS[variant] || vS.default} ${className}`} {...props}>{Icon && <Icon size={18} strokeWidth={2} className="shrink-0" />}<span className="truncate tracking-wide">{children}</span></button>);
});
...
const ClientCardItem = React.memo(({ c, idx, isExpanded, onToggleExpand, utils, openModal, onDeleteClient }) => {
    ...
    const handleAction = useCallback((msg) => (e) => { e.stopPropagation(); utils.openWhatsAppBusiness(phoneClean, msg); }, [phoneClean, utils]);
    ...
    return (
        <div className={`${GLASS_CARD} flex flex-col relative overflow-hidden transition-transform duration-200 hover:-translate-y-1 animate-fadeInUp`} style={{ animationFillMode: 'both', animationDelay: `${idx * 20}ms` }}>
            <div onClick={handleExpand} className="p-5 sm:p-6 cursor-pointer flex items-center justify-between gap-4 relative z-10 bg-transparent hover:bg-white/5 transition-colors duration-200">
                <div className="flex items-center gap-4 flex-1 min-w-0"><div className={`w-12 h-12 rounded-2xl flex items-center justify-center font-bold text-xl text-white shrink-0 shadow-lg bg-gradient-to-tr ${grad}`}>{c.isVIP ? <Award size={20} className="drop-shadow-md" /> : String(c.nombre).charAt(0).toUpperCase()}</div><div className="flex-1 min-w-0"><h4 className="font-bold text-[17px] text-white/90 capitalize truncate tracking-tight mb-1">{String(c.nombre)}</h4><p className="text-xs font-semibold text-white/60 flex items-center gap-1.5"><Smartphone size={14}/> {String(c.telefono) || 'Sin número'}</p></div></div>
                <div className="text-right shrink-0"><p className="text-xl font-bold text-emerald-400 leading-none tracking-tight">${c.totalGastado.toFixed(0)}</p><div className="flex justify-end gap-1.5 mt-2.5">{c.isVIP && <span className="w-2 h-2 rounded-full bg-amber-400" title="VIP"></span>}{c.isFrecuente && <span className="w-2 h-2 rounded-full bg-indigo-400" title="Frecuente"></span>}{c.isNuevo && <span className="w-2 h-2 rounded-full bg-emerald-400" title="Nuevo"></span>}{c.needsContact && <span className="w-2 h-2 rounded-full bg-rose-400" title="Contactar"></span>}</div></div>
            </div>
            {isExpanded && (
                <div className="relative z-10 px-5 pb-5 animate-fadeIn border-t border-white/5 mt-1 pt-4">
                    ...
                    <div className="flex gap-3 mb-4">
                        <ActionBtn icon={Sparkles} label="Promo" color="white" onClick={handleAction(msgPromo)} />
                        <ActionBtn icon={RefreshCw} label="Seguir" color="white" onClick={handleAction(msgSeguimiento)} />
                        <ActionBtn icon={BellRing} label="Recordar" color="white" onClick={handleAction(msgRecordatorio)} />
                    </div>
                    <div className="flex gap-3">
                        <AppButton variant="primary" icon={Plus} onClick={(e) => {e.stopPropagation(); openModal()}} className="flex-1 text-[13px] uppercase tracking-wider">Reservar</AppButton>
                        <button type="button" onClick={(e) => {e.stopPropagation(); onDeleteClient(c.nombre, c.eventos)}} className="px-5 bg-rose-500/10 text-rose-400 rounded-[16px] hover:bg-rose-500/20 transition-colors border border-rose-500/20"><Trash2 size={20} /></button>
                    </div>
                </div>
            )}
        </div>
        ...
        <div className="group">
            ...
            {isExpanded && (
                <div className="mx-5 mb-3 p-5 bg-[#0F172A] rounded-[16px] border border-white/5 animate-fadeIn">
                    ...
                    {ev.detalleGastos && (<div className="mt-4 pt-4 border-t border-white/5"><span className="text-[11px] uppercase font-semibold tracking-wider text-white/50 block mb-2">Desglose:</span><p className="text-[13px] font-medium text-white/70 italic leading-relaxed">{String(ev.detalleGastos)}</p></div>)}
                </div>
            )}
        </div>
    );
...
const EventCardItem = React.memo(({ ev, idx, todayTime, onWhatsApp, onViewDoc, onEdit, onDelete, onDuplicate, onMapClick, empresa, utils, onUpdateEstado, onConvertir }) => {
    ...
    else if (isCotizacion) { sideColor = 'bg-amber-300'; dotColor = 'bg-amber-300'; waType = 'cotizacion'; }
    else if (estNormalized === 'confirmado') { sideColor = 'bg-blue-500'; dotColor = 'bg-blue-400'; waType = 'recordatorio'; }
    else if (estNormalized === 'pendiente') { sideColor = 'bg-amber-500'; dotColor = 'bg-amber-400'; waType = 'cobro'; }
    ...
    if (diff === 0 && !isCotizacion) dateBadgeContent = <Badge color="rose"><div className="w-1.5 h-1.5 rounded-full bg-rose-500"></div> HOY</Badge>;
    else if (diff === 1 && !isCotizacion) dateBadgeContent = <Badge color="amber">MAÑANA</Badge>;
    else if (isCotizacion) {
        if (estNormalized.includes('aprobada')) dateBadgeContent = <Badge color="teal">COT. Aprobada</Badge>;
        else if (estNormalized.includes('rechazada')) dateBadgeContent = <Badge color="gray">COT. Rechazada</Badge>;
        else dateBadgeContent = <Badge color="amberSolid"><FileText size={12}/> Cotización</Badge>;
    }
                ...
                <div className="pl-3">
                    <div className="flex justify-between items-center gap-4">
                        <div className="flex items-center gap-2 sm:gap-3 min-w-0 flex-wrap">
                            <div className="flex items-center gap-2 min-w-0"><div className={`w-2.5 h-2.5 rounded-full ${dotColor} shrink-0`}></div><h3 className="text-lg font-bold text-white/90 truncate tracking-tight">{String(ev.cliente)}</h3></div>
                            <div className="flex items-center gap-2 shrink-0 mt-1 sm:mt-0">{dateBadgeContent}{ev.hora && (<Badge color="blue"><Clock size={12} strokeWidth={2.5}/> {utils.formatTime12h(ev.hora)}</Badge>)}</div>
                        </div>
                        {!isExpanded && (
                            <div className="flex items-center gap-4 shrink-0"><span className="text-white/90 font-bold text-lg tracking-tight">${tot.toFixed(2)}</span>{isCotizacion ? null : (restante > 0 ? (<div className="bg-rose-500/10 text-rose-400 px-3 py-1 rounded-lg text-xs font-bold uppercase tracking-wider border border-rose-500/20">Debe ${restante.toFixed(0)}</div>) : (<div className="flex items-center gap-1.5 text-emerald-400"><CheckCircle2 size={16} strokeWidth={2.5}/><span className="text-xs font-bold uppercase tracking-wider hidden sm:inline">Pagado</span></div>))}</div>
                        )}
                    </div>
                    <div className={`grid transition-all duration-300 ease-[cubic-bezier(0.16,1,0.3,1)] ${isExpanded ? 'grid-rows-[1fr] opacity-100 mt-6' : 'grid-rows-[0fr] opacity-0 mt-0'}`}>
                        <div className="overflow-hidden">
                            <div className="flex flex-col gap-4 mb-6 pt-2">
                                ...
                                <div className="flex items-center gap-4 text-white/70"><Calendar size={18} className="text-white/40 shrink-0" strokeWidth={2} /><span className="text-sm font-medium">{ev.fecha ? String(ev.fecha).split('-').reverse().join('/') : 'Sin fecha'} • {ev.hora ? utils.formatTime12h(ev.hora) : 'Sin hora'}</span></div>
                                <div onClick={(e) => { e.stopPropagation(); onMapClick(ev.direccion, ev.ubicacion); }} className="flex items-center justify-between gap-4 text-white/70 cursor-pointer hover:bg-white/5 p-2 -mx-2 rounded-xl transition-colors active:scale-[0.98] border border-transparent hover:border-white/5" title="Abrir en Google Maps">
                            ...
                            <div className="flex flex-col sm:flex-row gap-3">
                                <AppButton onClick={(e) => { e.stopPropagation(); onWhatsApp(ev, waType, empresa); }} className="w-full sm:flex-1 bg-gradient-to-r from-emerald-500 to-teal-500 hover:from-emerald-400 hover:to-teal-400 border-emerald-400/50 shadow-sm" icon={MessageCircle}>Contactar</AppButton>
        ...
        <div className="fixed inset-0 z-[9998] bg-black/70 flex justify-center items-end sm:items-center p-0 sm:p-4 animate-fadeIn">
            <div className={`${modalSectionClass} w-full h-[92vh] sm:h-auto sm:max-h-[90vh] sm:max-w-2xl flex flex-col overflow-hidden p-0 sm:p-0`}>
                 <div className="p-6 sm:p-8 border-b border-white/5 flex justify-between items-center z-20 bg-transparent">
                    <h3 className="font-bold text-white/90 text-2xl flex items-center gap-3 tracking-tight">{isCotizacionMode ? <FileText className="text-amber-400"/> : (initialData?.id && !initialData?.isDuplicated ? <Edit className="text-blue-400"/> : <Plus className="text-blue-400"/>)} {isCotizacionMode ? (initialData?.id ? 'Editar Cotización' : 'Nueva Cotización') : (initialData?.id && !initialData?.isDuplicated ? 'Editar Reserva' : 'Nueva Reserva')}</h3>
                 ...
                 <div className="overflow-y-auto flex-1 p-5 sm:p-8">
                  <form onSubmit={handleSubmit} className="max-w-xl mx-auto pb-8 space-y-6">
                     <div className={`${GLASS_CARD} p-6`}>
                        <div className="flex items-center gap-3 mb-6"><IconBox icon={Users} color="blue" /><h4 className="font-bold text-white/90 text-lg tracking-tight">Datos del Cliente</h4></div>
                        <div className="space-y-5">
                            <Field ref={nameInputRef} label="Nombre *" required value={formData.cliente} onChange={e=>setFormData({...formData,cliente:e.target.value})} />
                     ...
                     <div className={`${GLASS_CARD} p-6`}>
                        <div className="flex items-center gap-3 mb-6"><IconBox icon={MapPin} color="rose" /><h4 className="font-bold text-white/90 text-lg tracking-tight">Logística</h4></div>
                        ...
                        <div className="mb-5"><Field as="select" label="Zona" value={formData.ubicacion} onChange={handleZoneChange}>{Object.keys(ZONAS_TRANSPORTE).map(z => <option key={z} value={z} className="bg-[#0B1221] text-white">{z}</option>)}</Field></div>
                     ...
                     <div className={`${GLASS_CARD} p-6 relative z-30`}>
                        <div className="flex items-center gap-3 mb-6"><IconBox icon={Sparkles} color="amber" /><h4 className="font-bold text-white/90 text-lg tracking-tight">Servicios</h4></div>
                        <div className="relative mb-6 z-20">
                            <div className="flex items-center relative group"><Search className="absolute left-4 text-white/40 group-focus-within:text-blue-400 transition-colors" size={18} /><input type="text" value={searchTermService} onChange={(e) => { setSearchTermService(e.target.value); setShowDropdown(true); }} onFocus={() => setShowDropdown(true)} onBlur={() => setTimeout(() => setShowDropdown(false), 200)} placeholder="Buscar o agregar servicio..." className={`${inputClass} pl-12`} />{searchTermService && (<button type="button" onMouseDown={() => { setSearchTermService(''); setShowDropdown(false); }} className="absolute right-4 text-white/40 hover:text-white transition-colors"><X size={16}/></button>)}</div>
                            {showDropdown && (
                                <div className="absolute top-full left-0 right-0 mt-2 bg-[#0B1221] border border-white/5 rounded-xl shadow-2xl max-h-60 overflow-y-auto z-50">
                                    {filteredPaquetes.map(p => (<button type="button" key={p.id} onMouseDown={(e) => { e.preventDefault(); addService(p); setSearchTermService(''); setShowDropdown(false); }} className="w-full text-left px-5 py-4 hover:bg-white/5 border-b border-white/5 last:border-0 flex justify-between items-center transition-colors"><span className="font-medium text-white/90">{String(p.nombre)}</span><span className="text-emerald-400 font-bold">${utils.safeNum(p.precio)}</span></button>))}
                                    ...
                                    <div className="p-2 border-t border-white/5 sticky bottom-0 bg-[#0B1221]"><button type="button" onMouseDown={(e) => { e.preventDefault(); setIsCustomOpen(true); setShowDropdown(false); setSearchTermService(''); }} className="w-full py-3 bg-blue-500/10 text-blue-400 rounded-[12px] font-bold text-xs uppercase tracking-wider hover:bg-blue-500/20 transition-colors active:scale-[0.98]">+ Crear nuevo servicio</button></div>
                                </div>
                            )}
                        </div>

                        {isCustomOpen && (
                            <div className="mb-6 p-5 bg-[#0F172A] border border-white/5 rounded-2xl animate-fadeIn"><h5 className="font-bold text-white/90 text-sm mb-4">Servicio Personalizado</h5><div className="grid grid-cols-1 sm:grid-cols-2 gap-5 mb-5"><Field label="Nombre" value={customData.nombre} onChange={e=>setCustomData({...customData, nombre: e.target.value})} placeholder="Ej. Hora extra" /><Field label="Precio ($)" type="number" value={customData.precio} onChange={e=>setCustomData({...customData, precio: e.target.value})} placeholder="0.00" /></div><div className="flex gap-3 justify-end"><button type="button" onClick={() => setIsCustomOpen(false)} className="px-5 py-2.5 text-white/60 hover:text-white text-xs font-bold uppercase tracking-wider transition-colors">Cancelar</button><button type="button" onClick={handleCreateCustom} className="px-6 py-2.5 bg-blue-600 text-white rounded-xl text-xs font-bold uppercase tracking-wider hover:bg-blue-500 transition-colors active:scale-[0.98]">Agregar</button></div></div>
                        )}
                           ...
                           <div className="space-y-4 mb-2 pt-2 border-t border-white/5">
                               ...
                               {formData.serviciosSeleccionados.map((s, idx) => (
                                 <div key={idx} className="flex flex-col gap-4 p-5 bg-[#0F172A] rounded-[20px] border border-white/5 relative group hover:border-white/10 transition-colors duration-200">
                                    ...
                                    <div className="flex justify-between items-center pr-8"><span className="font-bold text-[15px] text-white/90 truncate">{String(s.nombre)}</span><div className="flex items-center bg-[#0B1221] rounded-xl p-1 border border-white/5"><button type="button" onClick={()=>updateServiceQuantity(idx,-1)} className="w-8 h-8 flex justify-center items-center hover:bg-white/5 rounded-lg text-white/50 hover:text-white transition-colors active:scale-[0.95]"><Minus size={14}/></button><span className="w-8 text-center font-bold text-white/90">{s.cantidad}</span><button type="button" onClick={()=>updateServiceQuantity(idx,1)} className="w-8 h-8 flex justify-center items-center hover:bg-white/5 rounded-lg text-white/50 hover:text-white transition-colors active:scale-[0.95]"><Plus size={14}/></button></div></div>
                                    <div className="grid grid-cols-1 gap-4 border-t border-white/5 pt-4"><Field label="Precio Modificable ($)" type="number" value={s.precio} onChange={(e) => handleServiceEdit(idx, 'precio', e.target.value)} className="w-full bg-[#0B1221] border border-white/5 rounded-xl px-4 py-3 text-[15px] font-medium text-white/90 outline-none focus:border-blue-500/50 transition-colors" /><Field as="textarea" label="Descripción para el PDF" value={s.descripcion || ''} onChange={(e) => handleServiceEdit(idx, 'descripcion', e.target.value)} rows={2} className="w-full bg-[#0B1221] border border-white/5 rounded-xl px-4 py-3 text-sm text-white/80 outline-none focus:border-blue-500/50 transition-colors resize-none leading-relaxed" placeholder="Detalles, viñetas, cambios..."/></div>
                                 </div>
                              ))}
                           </div>
                        )}
                     ...
                     <div className={`${GLASS_CARD} p-6`}>
                        <div className="flex items-center gap-3 mb-6"><IconBox icon={Receipt} color="emerald" /><h4 className="font-bold text-white/90 text-lg tracking-tight">Finanzas</h4></div>
                        
                        {!isCotizacionMode && (
                            <div className="grid grid-cols-2 gap-5 mb-6"><Field label="Abono" type="number" value={formData.abono} onChange={e=>setFormData({...formData,abono:e.target.value})} className={`${inputClass} text-emerald-400 font-bold`} /><Field label="Viáticos" type="number" value={formData.transporte} onChange={e=>{ const newTransporte = e.target.value; setFormData(prev => ({...prev, transporte: newTransporte, total: ((Array.isArray(prev.serviciosSeleccionados) ? prev.serviciosSeleccionados : []).reduce((sum, s) => sum + utils.safeNum(s.precio), 0) + utils.safeNum(newTransporte) + utils.safeNum(prev.gastos)).toString()})); }} /></div>
                        )}
                        ...
                        <div className="mb-6 border-t border-white/5 pt-6 mt-2">
                            ...
                            <div className="flex gap-2.5 overflow-x-auto pb-2 scrollbar-hide">
                                {opcionesEstado.map(est => (<button type="button" key={est} onClick={()=>setFormData({...formData,estado:est})} className={`shrink-0 flex-1 py-3.5 px-4 text-[10px] font-bold uppercase tracking-wider rounded-xl border transition-colors active:scale-[0.98] ${formData.estado===est ? 'bg-blue-600 text-white border-blue-500 shadow-md' : 'bg-[#0F172A] text-white/60 border-white/5 hover:bg-[#1E293B] hover:text-white/90'}`}>{est}</button>))}
                            </div>
                        </div>
                     ...
                     <AppButton variant="primary" icon={Check} onClick={handleSubmit} className="w-full py-4 text-base uppercase tracking-wider mt-2 mb-4">{isCotizacionMode ? 'Guardar Cotización' : 'Guardar Reserva'}</AppButton>
                  </form>
                 </div>
              </div>
        </div>
...
export default function App() {
  ...
  const [isAuthenticated, setIsAuthenticated] = useState(false); const [isAuthLoading, setIsAuthLoading] = useState(true);
  const [emailInput, setEmailInput] = useState(''); const [passwordInput, setPasswordInput] = useState('');
  const [firebaseUser, setFirebaseUser] = useState(null); const [activeTab, setActiveTab] = useState('inicio'); 
  const [isDBReady, setIsDBReady] = useState(false); const [eventos, setEventos] = useState([]); 
  const [paquetesPersonalizados, setPaquetesPersonalizados] = useState([]); const [hiddenClients, setHiddenClients] = useState([]);
  const [filterDate, setFilterDate] = useState(''); const [viewMode, setViewMode] = useState('semana'); 
  const [calMonth, setCalMonth] = useState(currentTime.getMonth()); const [calYear, setCalYear] = useState(currentTime.getFullYear());
  const [globalSearch, setGlobalSearch] = useState(''); const [isSyncing, setIsSyncing] = useState(false);
  ...
  const [isModoOperativo, setIsModoOperativo] = useState(false); const [isPrinting, setIsPrinting] = useState(false);
  const [printData, setPrintData] = useState(null); const [printType, setPrintType] = useState(null);
  const [pdfScale, setPdfScale] = useState(1); const [searchTerm, setSearchTerm] = useState('');
  const [clientSort, setClientSort] = useState('gasto'); const [financePeriod, setFinancePeriod] = useState('mes');
  const [expandedFinanceId, setExpandedFinanceId] = useState(null); const [expandedClientId, setExpandedClientId] = useState(null);
  const [isSidebarOpen, setIsSidebarOpen] = useState(false); const [messaging, setMessaging] = useState(null);
    ...
    try {
      const unsubscribe = onMessage(messaging, (payload) => {
        ...
        if (typeof Notification !== 'undefined' && Notification.permission === "granted" && navigator.serviceWorker) {
          navigator.serviceWorker.getRegistration().then(reg => { if (reg) reg.showNotification(title, { body: body, icon: "/icon-192.png" }); else new Notification(title, { body: body, icon: "/icon-192.png" }); }).catch(() => new Notification(title, { body: body, icon: "/icon-192.png" }));
        }
      });
      return () => unsubscribe();
    } catch (e) { console.warn("FCM foreground error", e); }
  }, [messaging, showAlert]);
  ...
  // NUEVO: Escalar automáticamente el PDF en móviles
  useEffect(() => {
    const handleResize = () => {
  ...
  const finanzasData = useMemo(() => {
      ...
      const bT = tI - tG; const roi = tI > 0 ? ((bT / tI) * 100).toFixed(0) : 0;
  ...
  const downloadPDF = useCallback(async () => {
    ...
    if (!window.html2pdf) { showAlert("Cargando generador, intenta en un segundo.", false); return; }
    const element = document.getElementById('pdf-content'); const wrapper = document.getElementById('pdf-wrapper-scaler');
    if (!element) { showAlert("Error al localizar el documento.", false); return; }
    showAlert("Generando PDF...", true);
    
    // Al generar el PDF, eliminamos la escala temporalmente para que html2pdf lo tome nítido en formato A4
    let oldTransform = ''; 
    if (wrapper) { 
    ...
    try {
        await new Promise(resolve => requestAnimationFrame(resolve)); await new Promise(resolve => setTimeout(resolve, 800));
        const docName = printData?.cliente ? String(printData.cliente).replace(/[^a-z0-9]/gi, '_') : 'Documento'; const fileName = `${printType === 'cotizacion' ? 'Cotizacion' : 'Factura'}_Diverty_${docName}.pdf`;
        const opt = { margin: 0, filename: fileName, image: { type: 'jpeg', quality: 1.0 }, html2canvas: { scale: 2, useCORS: true, backgroundColor: "#ffffff", width: 794, windowWidth: 794 }, jsPDF: { unit: 'px', format: [794, 1123], orientation: 'portrait' } };
        await window.html2pdf().set(opt).from(element).save(); showAlert("¡PDF descargado con éxito!", true); 
    } catch (error) { console.error("Error PDF:", error); showAlert("Error en descarga. Imprimiendo como respaldo...", false); printNativePDF(); } 
    finally { 
  ...
  const handleSharePDF = useCallback(async () => {
    utils.triggerHaptic('success');
    if (!window.html2pdf) { showAlert("Cargando generador...", false); return; }
    const element = document.getElementById('pdf-content'); const wrapper = document.getElementById('pdf-wrapper-scaler'); if (!element) return;
    showAlert("Preparando PDF para compartir...", true);
    let oldTransform = ''; if (wrapper) { oldTransform = wrapper.style.transform; wrapper.style.transform = 'scale(1)'; }
    const oldScrollY = window.scrollY; window.scrollTo(0, 0);
    try {
        await new Promise(resolve => requestAnimationFrame(resolve)); await new Promise(resolve => setTimeout(resolve, 800));
        const docName = printData?.cliente ? String(printData.cliente).replace(/[^a-z0-9]/gi, '_') : 'Documento'; const fileName = `${printType === 'cotizacion' ? 'Cotizacion' : 'Factura'}_Diverty_${docName}.pdf`;
        ...
        const file = new File([pdfBlob], fileName, { type: 'application/pdf' }); const msg = getWhatsAppMessage(printData, printType === 'cotizacion' ? 'cotizacion' : 'recibo', appSettings.empresa);
        if (navigator.canShare && navigator.canShare({ files: [file] })) { await navigator.share({ files: [file], title: fileName, text: msg }); } else { showAlert("No se pudo compartir directamente. Descargando...", false); await window.html2pdf().set(opt).from(element).save(); const phoneClean = String(printData?.telefono || '').replace(/\D/g,''); utils.openWhatsAppBusiness(phoneClean, msg); }
     ...
     if (isModoOperativo) {
        ...
        return (
          <div className="animate-fadeIn p-4 md:p-10 max-w-2xl mx-auto space-y-6 pb-32 relative z-50">
             ...
             <div className="flex justify-between items-center bg-gradient-to-r from-blue-600 to-indigo-600 text-white p-6 rounded-[24px] shadow-[0_10px_40px_rgba(37,99,235,0.3)] border border-white/10">
                 ...
                 <button type="button" onClick={() => setIsModoOperativo(false)} className="bg-white/10 hover:bg-white/20 p-3.5 rounded-xl transition-all shadow-sm border border-white/10"><X size={24} /></button>
             </div>
             {(faltanAbono.length > 0 || faltanDireccion.length > 0 || faltanHora.length > 0) && (
                 <div className="bg-white/5 border border-white/5 p-6 rounded-[24px] shadow-lg">
                     ...
                     <div className="space-y-4">
                         {faltanAbono.length > 0 && (<div className="flex items-center gap-4 bg-rose-500/10 border border-rose-500/20 p-4 rounded-xl"><IconBox icon={DollarSign} color="rose" className="border-0 p-2.5"/><div><p className="text-white font-bold text-[15px] leading-tight">Falta abono ({faltanAbono.length})</p><p className="text-rose-400 text-xs font-bold uppercase tracking-[0.1em] truncate max-w-[200px] mt-1">{faltanAbono.map(e=>String(e.cliente).split(' ')[0]).join(', ')}</p></div></div>)}
                         {faltanDireccion.length > 0 && (<div className="flex items-center gap-4 bg-amber-500/10 border border-amber-500/20 p-4 rounded-xl"><IconBox icon={MapPin} color="amber" className="border-0 p-2.5"/><div><p className="text-white font-bold text-[15px] leading-tight">Falta dirección ({faltanDireccion.length})</p><p className="text-amber-400 text-xs font-bold uppercase tracking-[0.1em] truncate max-w-[200px] mt-1">{faltanDireccion.map(e=>String(e.cliente).split(' ')[0]).join(', ')}</p></div></div>)}
                         {faltanHora.length > 0 && (<div className="flex items-center gap-4 bg-blue-500/10 border border-blue-500/20 p-4 rounded-xl"><IconBox icon={Clock} color="blue" className="border-0 p-2.5"/><div><p className="text-white font-bold text-[15px] leading-tight">Falta hora ({faltanHora.length})</p><p className="text-blue-400 text-xs font-bold uppercase tracking-[0.1em] truncate max-w-[200px] mt-1">{faltanHora.map(e=>String(e.cliente).split(' ')[0]).join(', ')}</p></div></div>)}
                     </div>
                 </div>
             )}
     ...
     return (
       <div className="animate-fadeIn p-4 md:p-6 lg:p-10 max-w-5xl mx-auto space-y-8 pb-32 md:pb-10">
          <div className="bg-gradient-to-br from-blue-600 via-indigo-600 to-purple-700 rounded-[32px] p-8 sm:p-12 shadow-[0_15px_40px_rgba(37,99,235,0.4)] border border-white/10 relative overflow-hidden group animate-fadeInUp">
             <div className="absolute inset-0 bg-[url('https://www.transparenttextures.com/patterns/cubes.png')] opacity-10 mix-blend-overlay pointer-events-none"></div><div className="absolute top-0 right-0 w-96 h-96 bg-[radial-gradient(circle,rgba(255,255,255,0.1)_0%,transparent_60%)] pointer-events-none"></div>
             <div className="relative z-10 text-center sm:text-left w-full sm:w-auto"><h1 className="text-3xl sm:text-5xl font-extrabold mb-3 flex items-center justify-center sm:justify-start gap-3 tracking-tight">Hola Diverty 👋</h1><p className="text-blue-100 font-medium text-sm sm:text-base tracking-wide">Gestiona tus reservas, contratos y finanzas al instante.</p></div>
             <div className="w-full sm:w-auto relative z-10 mt-6 sm:mt-0"><button onClick={() => openModal()} className="w-full sm:w-auto py-4 px-8 text-[15px] bg-white text-blue-900 hover:bg-slate-50 font-bold rounded-xl transition-all shadow-lg flex items-center justify-center gap-2.5"><Plus size={20} strokeWidth={2.5}/> Nueva Reserva</button></div>
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-4">
             <button type="button" onClick={() => openModal(null, true)} className="flex-1 bg-white/5 border border-white/5 text-white/90 rounded-xl py-4 font-bold flex items-center justify-center gap-2.5 hover:bg-white/10 transition-all shadow-sm"><FileText size={20} className="text-amber-400"/> <span className="hidden sm:inline">Crear Cotización</span></button>
             <button type="button" onClick={() => {utils.triggerHaptic('light'); setIsModoOperativo(true); window.scrollTo(0,0);}} className="flex-1 bg-blue-600 hover:bg-blue-500 text-white rounded-xl py-4 font-bold flex items-center justify-center gap-2.5 transition-all shadow-[0_8px_20px_rgba(37,99,235,0.3)]"><Zap size={20} className="text-amber-300 fill-amber-300"/> <span className="hidden sm:inline">Modo Operativo</span></button>
             <button type="button" onClick={() => { window.location.reload(); }} className="bg-white/5 border border-white/5 text-blue-400 rounded-xl py-4 px-6 flex items-center justify-center hover:bg-white/10 transition-all shadow-sm"><RefreshCw size={22} className={isSyncing ? "animate-spin" : ""} /></button>
          </div>
            ...
            <div className="animate-slideDown mt-10">
               ...
               <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                   {stats.alertasOperativas.map((al, i) => {
                      ...
                      return (
                      <div key={al.id} onClick={() => openModal(al.ev)} className={`p-5 sm:p-6 rounded-[24px] border flex items-center justify-between cursor-pointer transition-all ease-out bg-white/5 hover:border-rose-400/50 border-white/10 shadow-[0_4px_15px_rgba(0,0,0,0.2)] hover:shadow-lg animate-fadeInUp hover:bg-white/10`} style={{animationDelay: `${i*100}ms`}}>
        ...
        return (
            <div className={`bg-[#0B1221] rounded-[32px] border border-white/5 p-5 sm:p-8 mb-8 animate-fadeIn transition-all duration-300 shadow-[0_8px_30px_rgba(0,0,0,0.4)]`}>
               <div className="flex flex-col sm:flex-row justify-between items-center gap-5 mb-8 border-b border-white/5 pb-6">
                   <h3 className="text-2xl font-extrabold text-white/90 capitalize flex items-center gap-3 tracking-tight"><IconBox icon={CalendarDays} color="blue" /> {monthNames[calMonth]} {calYear}</h3>
                   <div className="flex gap-2 bg-[#060B14] p-1.5 rounded-[16px] border border-white/5 w-full sm:w-auto justify-between sm:justify-start shadow-inner">
                       <button type="button" onClick={() => { utils.triggerHaptic('light'); setCalMonth(calMonth === 0 ? 11 : calMonth - 1); setCalYear(calMonth === 0 ? calYear - 1 : calYear); }} className="p-3 hover:bg-white/5 rounded-xl transition-all shadow-sm text-white/50 hover:text-white"><ChevronLeft size={18}/></button>
                       <button type="button" onClick={() => { utils.triggerHaptic('light'); setCalMonth(todayObj.getMonth()); setCalYear(todayObj.getFullYear()); setFilterDate(todayStr); }} className="px-6 py-2 hover:bg-white/5 text-blue-400 font-bold text-xs uppercase tracking-[0.1em] rounded-xl transition-all shadow-sm">HOY</button>
                       <button type="button" onClick={() => { utils.triggerHaptic('light'); setCalMonth(calMonth === 11 ? 0 : calMonth + 1); setCalYear(calMonth === 11 ? calYear + 1 : calYear); }} className="p-3 hover:bg-white/5 rounded-xl transition-all shadow-sm text-white/50 hover:text-white"><ChevronRight size={18}/></button>
                   </div>
               </div>
               <div className="grid grid-cols-7 gap-2 sm:gap-4 text-center mb-4">{weekDays.map(d => <div key={d} className="text-[10px] sm:text-xs font-bold uppercase tracking-[0.1em] text-white/40">{d.substring(0,3)}</div>)}</div>
                     ...
                     return (
                         <div key={d} onClick={() => { utils.triggerHaptic('light'); setFilterDate(dateStr); setViewMode(''); }} className={`min-h-[70px] sm:min-h-[130px] p-2 sm:p-3 rounded-[16px] border transition-all duration-200 ease-out cursor-pointer flex flex-col justify-start items-center sm:items-start hover:-translate-y-0.5 active:scale-[0.98] ${isSelected ? 'border-blue-500 bg-blue-500/10 shadow-md' : isToday ? 'bg-rose-500/5 border-rose-500/20' : 'bg-[#060B14]/50 border-white/5 hover:border-white/10'}`}>
                            ...
                            <div className="mt-2 sm:mt-3 flex flex-wrap sm:flex-col gap-1 sm:gap-1.5 w-full justify-center sm:justify-start flex-1 overflow-hidden">
                                {hasEvents && <div className="hidden sm:flex flex-col gap-1.5 w-full">{dayEvents.slice(0, 2).map((ev, i) => (<div key={i} className="text-[9px] font-bold uppercase tracking-wider px-2 py-1 rounded-[8px] truncate bg-white/10 border border-white/5 text-white/90 w-full shadow-sm" title={ev.cliente}>{String(ev.cliente).split(' ')[0]}</div>))}{dayEvents.length > 2 && <div className="text-[9px] text-blue-400 font-bold uppercase tracking-wider mt-0.5 text-center w-full">+{dayEvents.length - 2}</div>}</div>}
                                {hasEvents && <div className="sm:hidden flex gap-1.5 mt-1 justify-center flex-wrap">{dayEvents.slice(0, 3).map((_, i) => <div key={i} className={`w-1.5 h-1.5 rounded-full ${isSelected ? 'bg-blue-400' : 'bg-white/60'}`}></div>)}{dayEvents.length > 3 && <div className="w-1.5 h-1.5 rounded-full bg-white/30"></div>}</div>}
                            </div>
                         </div>
                     );
                  })}
            ...
            <div className="flex flex-col lg:flex-row gap-5 mt-6">
                 ...
                 <div className="flex gap-3 overflow-x-auto scrollbar-hide py-1 items-center">
                     ...
                     <div className={`flex items-center justify-between px-5 py-3 rounded-[14px] transition-all duration-200 ease-out cursor-text focus-within:border-blue-500/50 bg-white/5 border ${filterDate ? 'border-blue-500/50 text-blue-400 shadow-sm' : 'border-white/5 shadow-sm hover:shadow-md text-white/60'} shrink-0`}>
                         ...
                         {filterDate && <button type="button" onClick={() => {utils.triggerHaptic('light'); setFilterDate('');}} className="text-white/40 hover:text-rose-400 ml-3 z-30 transition-all"><X size={16}/></button>}
                     </div>
                 </div>
            </div>
        </div>
                    ...
                    {contactCandidates.map((c, idx) => {
                        ...
                        return (
                           <div key={`contact-${c.nombre}`} className="snap-center shrink-0 w-80 bg-[#0B1221] p-6 rounded-[24px] border border-white/5 shadow-[0_8px_30px_rgba(0,0,0,0.4)] flex flex-col gap-5 animate-fadeInUp" style={{ animationDelay: `${idx * 100}ms` }}>
                              <div><p className="font-extrabold text-white/90 truncate text-xl tracking-tight capitalize">{c.nombre}</p><Badge color="rose" className="mt-2"><Clock size={12}/> Sin compras hace {c.daysSince} días</Badge></div>
                              <button onClick={() => { utils.openWhatsAppBusiness(phoneClean, msg); }} className="w-full bg-gradient-to-r from-emerald-500 to-teal-500 hover:from-emerald-400 hover:to-teal-400 text-white py-3.5 rounded-[16px] transition-all font-bold text-xs uppercase tracking-[0.1em] flex justify-center items-center gap-2 shadow-[0_8px_20px_rgba(16,185,129,0.3)]"><MessageCircle size={18} strokeWidth={2.5}/> Enviar Promo</button>
                           </div>
                        )
                    })}
                 </div>
          ...
          <div className="animate-fadeIn p-4 md:p-8 lg:p-10 max-w-5xl mx-auto space-y-8 pb-32">
             <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-4">
                 ...
                 <div className="flex gap-2 items-center bg-white/5 p-1.5 rounded-[16px] border border-white/5 shadow-inner w-full sm:w-auto">
                    <button type="button" onClick={() => {utils.triggerHaptic('light'); setFinancePeriod('mes');}} className={`flex-1 sm:flex-none px-6 py-3 rounded-xl font-bold text-[11px] uppercase tracking-wider transition-all ${financePeriod === 'mes' ? 'bg-gradient-to-r from-blue-600 to-indigo-600 text-white shadow-[0_4px_15px_rgba(37,99,235,0.4)]' : 'text-white/50 hover:text-white/90'}`}>Este Mes</button>
                    <button type="button" onClick={() => {utils.triggerHaptic('light'); setFinancePeriod('todos');}} className={`flex-1 sm:flex-none px-6 py-3 rounded-xl font-bold text-[11px] uppercase tracking-wider transition-all ${financePeriod === 'todos' ? 'bg-gradient-to-r from-blue-600 to-indigo-600 text-white shadow-[0_4px_15px_rgba(37,99,235,0.4)]' : 'text-white/50 hover:text-white/90'}`}>Histórico</button>
                    <div className="w-px h-6 bg-white/10 mx-1"></div>
                    <button type="button" onClick={downloadExcel} className="p-2 sm:px-5 sm:py-3 hover:bg-emerald-500/10 text-emerald-400 rounded-xl transition-all flex items-center gap-2" title="Exportar a Excel"><Download size={20} strokeWidth={2.5}/> <span className="hidden sm:inline text-[11px] font-bold uppercase tracking-wider">Excel</span></button>
                 </div>
             </div>
             ...
             <div className="bg-gradient-to-br from-blue-900/40 to-indigo-900/40 rounded-[32px] p-8 sm:p-12 shadow-[0_15px_40px_rgba(0,0,0,0.4)] relative overflow-hidden border border-white/10 animate-slideDown">
                 <div className="absolute -top-32 -left-32 w-64 h-64 bg-[radial-gradient(circle,rgba(59,130,246,0.15)_0%,transparent_60%)] pointer-events-none transform-gpu"></div><div className="absolute -bottom-32 -right-32 w-64 h-64 bg-[radial-gradient(circle,rgba(168,85,247,0.15)_0%,transparent_60%)] pointer-events-none transform-gpu"></div>
                 <div className="text-center relative z-10">
                     ...
                     <div className="flex flex-col sm:flex-row justify-center items-center gap-8 sm:gap-16 border-t border-white/10 pt-10 mt-2">
                         <div className="flex items-center gap-4"><IconBox icon={ArrowUpRight} color="emerald" /><div className="text-left"><p className="text-white/50 font-bold text-[10px] uppercase tracking-wider mb-1.5">Ingresos Brutos</p><p className="text-emerald-400 font-extrabold text-2xl leading-none tracking-tight">${finanzasData.tI.toFixed(2)}</p></div></div><div className="hidden sm:block w-px h-12 bg-white/10"></div>
                         <div className="flex items-center gap-4"><IconBox icon={ArrowDownRight} color="rose" /><div className="text-left"><p className="text-white/50 font-bold text-[10px] uppercase tracking-wider mb-1.5">Gastos Operativos</p><p className="text-rose-400 font-extrabold text-2xl leading-none tracking-tight">-${finanzasData.tG.toFixed(2)}</p></div></div><div className="hidden sm:block w-px h-12 bg-white/10"></div>
                         <div className="flex items-center gap-4"><IconBox icon={BarChart3} color="blue" /><div className="text-left"><p className="text-white/50 font-bold text-[10px] uppercase tracking-wider mb-1.5">Margen (ROI)</p><p className="text-blue-400 font-extrabold text-2xl leading-none tracking-tight">{finanzasData.roi}%</p></div></div>
                     </div>
                 </div>
             </div>
                 ...
                 <div className="col-span-2 bg-[#0B1221] p-6 sm:p-8 rounded-[24px] border border-white/5 shadow-lg flex items-end justify-between gap-4 h-[120px]">
                     <div className="flex-1 flex justify-between items-end h-full gap-2 sm:gap-3">
                         {chartData.map((d, i) => {
                             ...
                             return (<div key={i} className="w-full flex flex-col items-center justify-end h-full gap-1.5 group relative"><div className="absolute -top-8 bg-black/80 text-white text-[10px] font-bold px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity z-10 whitespace-nowrap">${d.value.toFixed(0)}</div><div className="w-full bg-blue-500/10 rounded-md relative overflow-hidden transition-all duration-300 ease-out group-hover:bg-blue-400/30 h-[70px]"><div className="absolute bottom-0 w-full bg-blue-50 transition-all duration-1000 ease-out shadow-[0_0_10px_rgba(59,130,246,0.8)]" style={{height: `${hPercent}%`}}></div></div><span className="text-[9px] font-bold uppercase text-white/40 tracking-widest">{String(d.date).split('-')[2]}</span></div>)
                         })}
                     </div>
             ...
             <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mt-10">
                <div className="flex flex-col gap-5 animate-fadeInUp" style={{animationDelay: '300ms'}}>
                   <div className="flex justify-between items-center px-2"><h4 className="font-extrabold text-xl text-white/90 flex items-center gap-3 tracking-tight"><Clock size={22} className="text-rose-400"/> Cuentas por Cobrar <span className="text-sm font-bold text-white/40 uppercase tracking-widest">{financePeriod === 'mes' ? '(Mes)' : '(Histórico)'}</span></h4>{evtCalculoBase.filter(e => (utils.safeNum(e.total)-utils.safeNum(e.abono))>0 && utils.normalizeText(e.estado)!=='completado').length > 0 && (<button onClick={handleCopiarCobros} className="text-[10px] font-bold uppercase tracking-wider text-blue-400 bg-blue-500/10 hover:bg-blue-500/20 py-2.5 px-5 rounded-[12px] transition-all border border-blue-500/20 flex items-center gap-2 shadow-sm"><Copy size={16}/> Copiar Lista</button>)}</div>
                      ...
                      {evtCalculoBase.filter(e => (utils.safeNum(e.total)-utils.safeNum(e.abono))>0 && utils.normalizeText(e.estado)!=='completado').length === 0 ? (<div className="flex-1 flex flex-col items-center justify-center p-8 opacity-60"><CheckCircle2 size={48} className="text-emerald-400 mb-4"/><p className="text-[11px] font-bold uppercase tracking-wider text-white/60">Excelente. Sin deudas pendientes.</p></div>) : (
                          <div className="overflow-y-auto flex-1 scrollbar-hide p-4 space-y-2">
                              {evtCalculoBase.filter(e => (utils.safeNum(e.total)-utils.safeNum(e.abono))>0 && utils.normalizeText(e.estado)!=='completado').map((ev, i) => (
                                   <div key={ev.id} className="w-full flex justify-between items-center p-5 rounded-[20px] bg-transparent hover:bg-white/5 transition-colors border border-transparent hover:border-white/5">
                ...
                <div className="flex flex-col gap-5 animate-fadeInUp" style={{animationDelay: '400ms'}}>
                   ...
                   <div className="bg-[#0B1221] rounded-[24px] border border-white/5 shadow-lg overflow-hidden flex flex-col h-[400px]">
                      {evtCalculoBase.length === 0 ? (<div className="flex-1 flex flex-col items-center justify-center p-8 opacity-60"><Info size={48} className="text-white/40 mb-4"/><p className="text-[11px] font-bold uppercase tracking-wider text-white/60">No hay movimientos en este periodo.</p></div>) : (
                          <div className="overflow-y-auto flex-1 scrollbar-hide p-4 space-y-2">{evtCalculoBase.map((ev, i) => (<TransactionItem key={ev.id} ev={ev} isExpanded={expandedFinanceId === ev.id} onToggleExpand={handleToggleFinance} utils={utils} />))}</div>
                      )}
                   </div>
                </div>
             </div>
  ...
  const renderConfig = () => {
    return (
      <div className="animate-fadeIn p-4 md:p-8 lg:p-10 max-w-7xl mx-auto space-y-6 pb-32">
          <div className="mb-8"><h2 className="text-4xl sm:text-5xl font-black text-white/90 tracking-tight drop-shadow-sm">Ajustes</h2><p className="text-base font-medium text-white/50 mt-2">Centro de Mando Diverty</p></div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 sm:gap-8">
              <div className="col-span-1 lg:col-span-2 bg-[#0B1121] rounded-[32px] p-8 sm:p-10 shadow-[0_10px_40px_rgba(0,0,0,0.3)] relative overflow-hidden flex flex-col justify-between border border-white/5 animate-fadeInUp">
                  <div className="absolute -top-32 -right-32 w-64 h-64 bg-[radial-gradient(circle,rgba(59,130,246,0.15)_0%,transparent_60%)] pointer-events-none transform-gpu"></div><div className="absolute -bottom-32 -left-32 w-64 h-64 bg-[radial-gradient(circle,rgba(99,102,241,0.15)_0%,transparent_60%)] pointer-events-none transform-gpu"></div>
                  <div className="relative z-10 flex flex-col sm:flex-row items-start sm:items-center gap-6">
                       <div className="w-24 h-24 rounded-[24px] bg-gradient-to-tr from-blue-500 to-indigo-500 p-[3px] shadow-xl shrink-0"><img src={LOGO_URL} className="w-full h-full object-contain rounded-[21px] bg-[#060B14] p-3" alt="Diverty Profile" crossOrigin="anonymous"/></div>
                       <div><h3 className="text-3xl font-extrabold text-white/90 tracking-tight">Administrador Global</h3><p className="text-blue-400 text-sm font-bold tracking-[0.1em] uppercase mt-2 flex items-center gap-2.5"><Cloud size={16} className="text-emerald-400"/> Sincronizado</p></div>
                  </div>
                  <div className="relative z-10 mt-10 flex flex-col sm:flex-row gap-5 border-t border-white/5 pt-8">
                       <button type="button" onClick={activarNotificaciones} className="flex-1 flex items-center justify-center gap-2.5 bg-blue-500/10 hover:bg-blue-500/20 text-blue-400 py-4 rounded-[16px] transition-all border border-blue-500/20 font-bold text-sm shadow-sm"><BellRing size={20}/> Obtener Token Push</button>
                       <button onClick={handleLogout} className="flex-1 flex items-center justify-center gap-2.5 bg-white/5 hover:bg-rose-500/10 text-white/70 hover:text-rose-400 py-4 rounded-[16px] transition-all border border-white/5 hover:border-rose-500/20 font-bold text-sm"><Lock size={20}/> Cerrar Sesión</button>
                  </div>
              </div>
              ...
              <div className={`${GLASS_CARD} p-8 sm:p-10 flex flex-col relative overflow-hidden animate-fadeInUp`} style={{animationDelay:'100ms'}}>
                  ...
                  <div className="relative z-10 flex-1 flex flex-col">
                      ...
                      <div className="relative mb-5"><span className="absolute left-5 top-1/2 -translate-y-1/2 text-white/40 font-extrabold text-xl">$</span><input type="number" value={appSettings.metaMensual} onChange={e => updateSettings({...appSettings, metaMensual: utils.safeNum(e.target.value)})} className="w-full bg-[#060B14] border border-white/5 rounded-[16px] py-4 pl-10 pr-5 text-2xl font-extrabold text-white/90 outline-none focus:border-blue-500/50 focus:ring-2 focus:ring-blue-500/20 transition-all shadow-[inset_0_2px_4px_rgba(0,0,0,0.2)]" /></div>
              ...
              <div className={`col-span-1 lg:col-span-3 ${GLASS_CARD} p-8 sm:p-10 animate-fadeInUp`} style={{animationDelay:'200ms'}}>
                  ...
                  <div className="mt-8 bg-white/5 p-5 rounded-[16px] border border-white/5 flex items-start gap-4"><Info size={20} className="text-white/40 shrink-0 mt-0.5"/><p className="text-xs font-medium text-white/60 leading-relaxed">Estos datos se insertarán automáticamente en todos los PDFs de contratos, facturas y en los mensajes de WhatsApp que envíes a tus clientes.</p></div>
              </div>

              <div className="col-span-1 lg:col-span-3 bg-rose-500/5 border border-dashed border-rose-500/30 rounded-[32px] p-8 sm:p-10 mt-4 animate-fadeInUp flex flex-col sm:flex-row items-center justify-between gap-6 transition-all hover:bg-rose-500/10" style={{animationDelay:'300ms'}}>
                  <div><h4 className="font-extrabold text-rose-400 text-2xl flex items-center gap-3 tracking-tight"><AlertTriangle size={28}/> Zona de Peligro</h4><p className="text-[15px] font-medium text-rose-300/70 mt-3 max-w-xl leading-relaxed">Esta acción purgará toda la base de datos local y en la nube. Se eliminarán todas las reservas, el historial de clientes y los registros financieros de forma permanente.</p></div>
                  <button onClick={handleWipeAll} className="w-full sm:w-auto bg-rose-600 hover:bg-rose-500 text-white font-bold py-5 px-8 rounded-[16px] shadow-[0_8px_20px_rgba(225,29,72,0.3)] transition-all uppercase tracking-[0.1em] text-[13px] flex items-center justify-center gap-3"><Trash2 size={20}/> Purgar Sistema</button>
              </div>
          </div>
      </div>
    );
  ...
  if (isPrinting && printData) {
    ...
    return (
      <div className="bg-slate-200 min-h-screen text-slate-900 flex flex-col font-sans overflow-x-hidden animate-fadeIn relative">
        ...
        {/* NUEVO: Contenedor que adapta el tamaño de la previsualización a cualquier pantalla */}
          ...
          <div style={{ width: `${794 * pdfScale}px`, height: `${1123 * pdfScale}px`, position: 'relative' }}>
              <div id="pdf-wrapper-scaler" style={{ transform: `scale(${pdfScale})`, transformOrigin: 'top left', width: '794px', position: 'absolute', top: 0, left: 0 }}>
                  <div id="pdf-content" className="bg-[#F9FAFB] w-[794px] min-h-[1123px] h-auto relative overflow-hidden font-sans text-[#111827] p-12 flex flex-col shadow-2xl">
                     <div className="absolute top-[-150px] left-[-100px] w-[400px] h-[400px] rounded-full bg-[#6366F1]/10 z-0"></div><div className="absolute top-[-100px] left-[-50px] w-[300px] h-[300px] rounded-full bg-[#4F46E5]/80 z-0"></div><div className="absolute bottom-[-150px] right-[-100px] w-[400px] h-[400px] rounded-full bg-[#A855F7]/10 z-0"></div><div className="absolute bottom-[-100px] right-[-50px] w-[300px] h-[300px] rounded-full bg-[#9333EA]/80 z-0"></div>
                     
                     <div className="flex flex-col mb-10 relative z-10">
                         <div className="flex justify-between items-start w-full"><div><img src={LOGO_URL} alt="Diverty" className="h-16 object-contain drop-shadow-md" crossOrigin="anonymous" /></div><div className="text-right"><p className="text-sm text-[#6B7280] mt-1 font-medium tracking-wide">Ref: {numRef} &nbsp;|&nbsp; Fecha: {fechaDoc}</p></div></div>
                         <h1 className="text-[28px] font-black text-[#4F46E5] tracking-widest uppercase text-center mt-6">{isC ? 'COTIZACIÓN' : (isFact ? 'FACTURA' : 'CONTRATO DE SERVICIOS')}</h1><div className="w-full h-[1px] bg-[#4F46E5]/20 mt-6"></div>
                     </div>
                     
                     <div className="flex justify-between gap-8 mb-12 relative z-10">
                        <div className="w-1/2 bg-white p-6 rounded-xl border border-[#4F46E5]/10 shadow-sm"><h3 className="text-[11px] font-bold text-[#4F46E5] uppercase tracking-widest mb-4 border-b border-gray-100 pb-3 flex items-center gap-2"><Users size={16}/> Datos del Cliente</h3><div className="space-y-3 text-[13px] font-medium text-[#6B7280]"><PdfItem label="Nombre:" value={cli} /><PdfItem label="Teléfono:" value={tel} />{emailStr && <PdfItem label="Email:" value={emailStr} className="break-all" />}{rucStr && <PdfItem label="RUC:" value={rucStr} />}</div></div>
                        <div className="w-1/2 bg-white p-6 rounded-xl border border-[#4F46E5]/10 shadow-sm"><h3 className="text-[11px] font-bold text-[#4F46E5] uppercase tracking-widest mb-4 border-b border-gray-100 pb-3 flex items-center gap-2"><MapPin size={16}/> Detalles del Evento</h3><div className="space-y-3 text-[13px] font-medium text-[#6B7280]"><PdfItem label="Fecha:" value={fechaDoc} /><PdfItem label="Horario:" value={horaStr} /><PdfItem label="Lugar:" value={ubi} className="truncate w-40" />{dir && <div className="text-xs italic text-right mt-1 line-clamp-2">{dir}</div>}</div></div>
                     </div>
                     ...
                     {isFact || isC ? (
                        <>
                             <div className="mb-6 flex-1 relative z-10">
                                <div className="border border-[#4F46E5]/20 rounded-xl overflow-hidden shadow-sm bg-white">
                                     <table className="w-full text-left text-[13px]">
                                         <thead className="bg-[#F9FAFB] border-b border-[#4F46E5]/10 text-[#4F46E5]"><tr><th className="py-4 px-6 font-bold uppercase text-center tracking-widest w-1/3 text-[11px]">Artículo</th><th className="py-4 px-6 font-bold uppercase tracking-widest text-center border-l border-gray-100 w-1/2 text-[11px]">Descripción</th><th className="py-4 px-6 font-bold uppercase text-center tracking-widest border-l border-gray-100 w-1/6 text-[11px]">Total</th></tr></thead>
                                         <tbody className="divide-y divide-gray-100">
                                             {sA.map((s, i) => { const cant = Number(s.cantidad) || 1; const precioUnitario = utils.safeNum(s.precio) / cant; return ( <tr key={i} className="avoid-break"><td className="py-6 px-6 text-center border-r border-gray-100 align-top"><div className="flex justify-center mb-3 text-[#4F46E5]"><Star size={28} strokeWidth={1.5}/></div><p className="font-bold text-[#111827] text-[14px] leading-tight">{String(s.nombre)}</p>{cant > 1 && <p className="font-semibold text-[#7C3AED] text-[12px] mt-1.5 bg-[#7C3AED]/10 py-1 rounded-md inline-block px-3">- {cant} Horas -</p>}<p className="text-[10px] text-[#6B7280] font-medium mt-2 tracking-wide uppercase">Animación Infantil</p></td><td className="py-6 px-8 border-r border-gray-100 align-top"><div className="text-[#6B7280] text-[12px] leading-relaxed space-y-2">{String(s.descripcion || 'Servicio de animación para eventos').split('\n').map((line, j) => { const tLine = String(line).trim(); if(tLine.startsWith('•') || tLine.startsWith('-')) return <div key={j} className="flex items-start gap-2.5 font-medium"><CheckCircle2 size={14} className="text-[#4F46E5] shrink-0 mt-[2px]"/> <span className="text-[#374151]">{tLine.replace(/^[•-]\s*/, '')}</span></div>; return <div key={j} className="mb-2">{tLine}</div>; })}</div></td><td className="py-6 px-6 text-center align-middle"><p className="font-black text-[#111827] text-[18px]">B/. {utils.safeNum(s.precio).toFixed(2)}</p><p className="text-[10px] text-[#6B7280] mt-1.5 font-medium">{cant.toFixed(2)} x B/. {precioUnitario.toFixed(2)}</p></td></tr> )})}
                                             {trn > 0 && (<tr className="avoid-break"><td className="py-6 px-6 text-center border-r border-gray-100 align-middle"><div className="flex justify-center mb-3 text-[#4F46E5]"><MapIcon size={28} strokeWidth={1.5}/></div><p className="font-bold text-[#111827] text-[14px]">Transporte</p></td><td className="py-6 px-8 border-r border-gray-100 align-middle text-[12px] text-[#374151] leading-relaxed font-medium"><div className="flex items-start gap-2.5"><CheckCircle2 size={14} className="text-[#4F46E5] shrink-0 mt-[2px]"/> <span>Cargo por viáticos a zona: {ubi}</span></div></td><td className="py-6 px-6 text-center align-middle"><p className="font-black text-[#111827] text-[18px]">B/. {trn.toFixed(2)}</p></td></tr>)}
                                         </tbody>
                                     </table>
                                </div>
                             </div>
                             ...
                             <div className="px-0 mt-4 flex justify-between gap-8 avoid-break relative z-10">
                                 <div className="w-1/2"><div className="bg-white p-5 rounded-xl border border-[#4F46E5]/10 shadow-sm h-full"><div className="flex items-center gap-2 mb-3 text-[#4F46E5] border-b border-gray-100 pb-3"><FileText size={18}/><h3 className="font-bold uppercase tracking-widest text-[11px]">Notas Importantes</h3></div><div className="text-[11px] font-medium text-[#6B7280] leading-relaxed space-y-2"><p>Gracias por confiar en Diverty Eventos Panamá.</p><p>El saldo pendiente debe ser cancelado antes o al finalizar el evento.</p><p>Para pagos aceptamos: Yappy, Transferencia Bancaria o Efectivo.</p></div></div></div>
                                 <div className="w-[45%] flex flex-col items-end"><div className="w-full border border-[#4F46E5]/10 rounded-xl overflow-hidden bg-white shadow-sm flex flex-col"><div className="flex justify-between items-center py-3.5 px-6 border-b border-gray-100 text-[13px]"><span className="font-semibold text-[#6B7280]">Subtotal:</span><span className="font-bold text-[#111827]">B/. {tot.toFixed(2)}</span></div>{!isC && abo > 0 && (<div className="flex justify-between items-center py-3.5 px-6 border-b border-gray-100 text-[13px] bg-emerald-50/50"><span className="font-semibold text-[#6B7280]">Abono:</span><span className="font-bold text-emerald-600">- B/. {abo.toFixed(2)}</span></div>)}<div className="bg-[#4F46E5] text-white py-6 px-6 text-center"><span className="block text-[11px] uppercase tracking-widest font-semibold mb-1 opacity-90">{isC ? 'Total Estimado:' : 'Total Pendiente:'}</span><span className="block text-[36px] font-black leading-none pb-1">B/. {isC ? tot.toFixed(2) : (tot - abo).toFixed(2)}</span></div></div></div>
                             </div>
                             ...
                             <div className="mt-8 pb-4 avoid-break relative z-10">
                                 <div className="bg-white rounded-xl border border-[#4F46E5]/10 p-6 flex justify-between items-stretch text-[11px] shadow-sm">
                                     <div className="flex gap-4 w-[35%] border-r border-gray-100 pr-4"><div className="text-[#4F46E5] shrink-0 mt-1"><Briefcase size={20}/></div><div className="leading-snug space-y-1.5"><h4 className="font-bold text-[#111827] uppercase tracking-widest mb-2 text-[10px]">Métodos de Pago</h4><p className="font-semibold text-[#111827]">Yappy: <span className="font-medium text-[#6B7280]">{appSettings.empresa.telefono}</span></p><p className="font-medium text-[#6B7280]">{appSettings.empresa.banco} - {appSettings.empresa.tipoCuenta}</p><p className="font-semibold text-[#111827]">Nº: <span className="font-medium text-[#6B7280]">{appSettings.empresa.numeroCuenta}</span></p><p className="font-semibold text-[#111827]">Titular: <span className="font-medium text-[#6B7280] uppercase">{appSettings.empresa.nombreTitular}</span></p></div></div>
                                     <div className="flex gap-4 w-[35%] border-r border-gray-100 px-6"><div className="text-[#4F46E5] shrink-0 mt-1"><CheckCircle2 size={20}/></div><div className="leading-snug"><h4 className="font-bold text-[#111827] uppercase tracking-widest mb-2 text-[10px]">Garantía de Servicio</h4><p className="font-medium text-[#6B7280] leading-relaxed text-[11px]">Nuestro compromiso es brindarte la mejor experiencia. Si tienes alguna duda o cambio, contáctanos con tiempo.</p></div></div>
                                     <div className="flex flex-col items-center justify-center w-[30%] pl-6"><div className="text-[#4F46E5] flex items-center gap-2 mb-2 font-bold text-[12px] uppercase tracking-widest"><Award size={18}/> ¡Gracias!</div><h3 className="text-2xl text-[#111827]" style={{fontFamily: "'Brush Script MT', 'Dancing Script', cursive, serif", fontStyle: "italic"}}>Diverty Eventos</h3><p className="text-[#6B7280] font-medium text-[10px] mt-1.5 text-center tracking-wide uppercase">Diversión que crea recuerdos</p></div>
                                 </div>
                             </div>
                        </>
                     ) : (
                        <>
                             <div className="mb-10 relative z-10"><h3 className="text-[11px] font-bold text-[#4F46E5] uppercase tracking-widest mb-5 flex items-center gap-2 justify-center"><Sparkles size={16}/> Servicios Contratados</h3><div className="bg-[#F3F4F6] px-8 py-5 rounded-full border border-gray-200 shadow-sm text-center"><p className="text-[14px] text-[#111827] font-semibold leading-relaxed capitalize">{servicioLimpioContrato}</p></div></div>
                             ...
                             <div className="mt-auto pt-16 flex justify-between items-end pb-8 px-12 avoid-break relative z-10">
                                 <div className="w-[40%] text-center"><div className="border-t border-gray-300 pt-4"><p className="font-semibold text-[13px] text-[#111827] uppercase truncate tracking-wide">{cli}</p><p className="text-[10px] font-medium text-[#6B7280] uppercase tracking-[0.2em] mt-1.5">Firma del Cliente</p></div></div>
                                 <div className="w-[40%] text-center"><div className="border-t border-gray-300 pt-4"><p className="font-semibold text-[13px] text-[#111827] uppercase truncate tracking-wide">{appSettings.empresa.nombreTitular}</p><p className="text-[10px] font-medium text-[#6B7280] uppercase tracking-[0.2em] mt-1.5">Diverty Eventos Panamá</p></div></div>
                             </div>
                        </>
                     )}
                  </div>
  ...
  if (isAuthLoading) return (<div className="font-outfit min-h-[100dvh] flex flex-col items-center justify-center bg-[#060B14]"><div className="w-12 h-12 border-4 border-blue-500/20 border-t-blue-500 rounded-full animate-spin mb-4"></div><p className="text-white/50 text-sm font-bold uppercase tracking-widest animate-pulse">Iniciando</p></div>);

  if (!isAuthenticated) return (
    <div className="font-outfit min-h-[100dvh] flex items-center justify-center p-4 relative overflow-hidden bg-[#060B14]">
      <div className="absolute top-[-10%] left-[-10%] w-[500px] h-[500px] bg-[radial-gradient(circle,rgba(37,99,235,0.15)_0%,transparent_60%)] pointer-events-none transform-gpu"></div><div className="absolute bottom-[-10%] right-[-5%] w-[400px] h-[400px] bg-[radial-gradient(circle,rgba(79,70,229,0.15)_0%,transparent_60%)] pointer-events-none transform-gpu"></div>
      <div className="w-full max-w-sm bg-[#0B1221] rounded-[24px] p-10 shadow-[0_20px_50px_rgba(0,0,0,0.5)] border border-white/5 relative z-10 animate-fadeInUp">
        <div className="flex justify-center mb-8"><div className="w-20 h-20 rounded-[20px] bg-gradient-to-tr from-blue-600 to-indigo-500 p-[2px] shadow-lg hover:scale-[1.02] transition-transform duration-200 ease-out"><div className="w-full h-full bg-[#060B14] rounded-[18px] flex items-center justify-center"><ShieldCheck size={32} className="text-white" strokeWidth={1.5} /></div></div></div>
        <div className="text-center mb-10"><h1 className="text-3xl font-extrabold text-white/90 mb-2 tracking-tight">Acceso Seguro</h1><p className="text-white/50 font-medium text-sm">Autenticación por Firebase</p></div>
        <form onSubmit={handleLogin} className="space-y-4">
          ...
          <button type="submit" className="w-full bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 text-white font-bold text-lg py-4 rounded-xl shadow-[0_4px_15px_rgba(37,99,235,0.3)] transition-all active:scale-[0.98] flex items-center justify-center gap-2 mt-4">Ingresar <ChevronRight size={20} strokeWidth={3} /></button>
        </form>
      </div>
    </div>
  );
    ...
    <div className={`font-outfit min-h-[100dvh] flex overflow-hidden selection:bg-blue-500/30 transition-colors duration-200 relative bg-[#060B14] text-slate-100`}>
      ...
      {confirmModal.isOpen && (
          <div className="fixed inset-0 z-[100000] bg-black/70 flex items-center justify-center p-4 animate-fadeIn overscroll-none">
              <div className={`${modalSectionClass} max-w-md w-full text-center border-rose-500/30 p-8`}><div className="w-20 h-20 bg-rose-500/10 rounded-full flex items-center justify-center mx-auto mb-6 border border-rose-500/20"><AlertTriangle size={32} className="text-rose-400" /></div><h3 className="text-2xl font-extrabold text-white/90 mb-3 tracking-tight">¿Estás seguro?</h3><p className="text-white/50 font-medium mb-8 leading-relaxed">{confirmModal.message}</p><div className="flex gap-4"><button onClick={() => setConfirmModal({ isOpen: false, message: '', onConfirm: null })} className="flex-1 py-3.5 rounded-xl font-bold uppercase tracking-wider text-white/70 bg-white/5 hover:bg-white/10 active:scale-[0.98] transition-all">Cancelar</button><button onClick={() => { if (confirmModal.onConfirm) confirmModal.onConfirm(); setConfirmModal({ isOpen: false, message: '', onConfirm: null }); }} className="flex-1 py-3.5 rounded-xl font-bold uppercase tracking-wider text-white bg-rose-600 hover:bg-rose-500 shadow-[0_4px_15px_rgba(225,29,72,0.3)] active:scale-[0.98] transition-all">Confirmar</button></div></div>
          </div>
      )}
